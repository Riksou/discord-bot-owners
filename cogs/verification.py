...
async def accept_verification(
    interaction: discord.Interaction, member: discord.Member, message: discord.Message
) -> None:
    await interaction.client.mongo.update_guild_member_document(
        member.id,
        {"$set": {"verification_pending": False, "verification_cooldown": None}},
    )
    ...
    general_channel = interaction.guild.get_channel(
        interaction.client.config["channel_id"]["general"]
    )
...
async def is_on_cooldown(interaction: discord.Interaction) -> bool:
    guild_member = await interaction.client.mongo.fetch_guild_member(
        interaction.user.id
    )
    ...
    if cooldown is not None and cooldown > now:
        ...
        await interaction.response.send_message(
            f"You are on cooldown for the verification system, please try again {remaining}.",
            ephemeral=True,
        )
        return True
...
async def apply_verification_submit_actions(
    ...
    await interaction.client.mongo.update_guild_data_document(
        {
            "$set": {
                f"pending_verification_message_ids.{pending_verification_message_id.id}": interaction.user.id
            }
        }
    )

    confirmation_message = (
        "Thanks for your request, please wait while we review your application."
    )
...
class AcceptedBotOwnerVerificationSelect(discord.ui.Select):
    def __init__(
        self, client: DiscordBotOwners, member: discord.Member, message: discord.Message
    ):
        self.member = member
        ...
        roles = [
            guild.get_role(int(d)) for d in client.config["role_id"]["bot_owner_roles"]
        ]
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        verified_member = interaction.guild.get_role(
            interaction.client.config["role_id"]["verified_member"]
        )
...
class DeniedBotOwnerVerificationModal(discord.ui.Modal, title="Deny Verification"):
    ...
    reason = discord.ui.TextInput(
        label="Reason", style=discord.TextStyle.paragraph, max_length=1024
    )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        await interaction.client.mongo.update_guild_member_document(
            user_id, {"$set": {"verification_pending": False}}
        )
        await interaction.client.mongo.update_guild_data_document(
            {
                "$unset": {
                    f"pending_verification_message_ids.{interaction.message.id}": ""
                }
            }
        )
...
class PendingVerificationView(discord.ui.View):
    ...
    @discord.ui.button(
        label="Accept", style=discord.ButtonStyle.green, custom_id="persisten:accept"
    )
    async def accept(
        ...
        user_id = guild_data["pending_verification_message_ids"][
        ...
        if len(embed.fields) == 5:
            ...
            view.add_item(
                AcceptedBotOwnerVerificationSelect(
                    interaction.client, member, interaction.message
                )
            )
    ...
    @discord.ui.button(
        label="Deny", style=discord.ButtonStyle.red, custom_id="persisten:deny"
    )
    async def deny(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ) -> None:
        await interaction.response.send_modal(
            DeniedBotOwnerVerificationModal(interaction.message)
        )


class BotOwnerModal(discord.ui.Modal, title="Apply as a Bot Owner"):
    application_id = discord.ui.TextInput(
        label="Bot ID", style=discord.TextStyle.short, max_length=1024
    )

    oauth_url = discord.ui.TextInput(
        label="OAuth URL", style=discord.TextStyle.short, max_length=1024
    )
    ...
    guild_count = discord.ui.TextInput(
        label="Guild Count", style=discord.TextStyle.short, max_length=1024
    )
...
class LibraryDeveloperModal(discord.ui.Modal, title="Apply as a Library Developer"):
    name = discord.ui.TextInput(
        label="Library's Name", style=discord.TextStyle.short, max_length=1024
    )

    github_link = discord.ui.TextInput(
        label="Library's GitHub link", style=discord.TextStyle.short, max_length=1024
    )
    ...
    async def on_submit(self, interaction: discord.Interaction) -> None:
        ...
        verification_embed.add_field(
            name="Library Name", value=self.name.value, inline=False
        )
        verification_embed.add_field(
            name="GitHub Link", value=self.github_link.value, inline=False
        )
        verification_embed.add_field(
            name="Support Server Invite",
            value=self.support_server_invite.value,
            inline=False,
        )
        verification_embed.add_field(name="Status", value="Pending", inline=False)
...
class BotTeamModal(discord.ui.Modal, title="Apply as a Bot Team Member"):
    ...
    code = discord.ui.TextInput(label="Code", style=discord.TextStyle.short)
    ...
    async def on_submit(self, interaction: discord.Interaction) -> None:
        try:
            ...
            return await interaction.response.send_message(
                "You must input a valid user ID.", ephemeral=True
            )
        ...
        if self.code.value not in guild_member["verification_codes"]:
            return await interaction.response.send_message(
                "You have entered an invalid code.", ephemeral=True
            )
        ...
        await interaction.client.mongo.update_guild_member_document(
            ...
            {
                "$set": {
                    "verification_join_code": self.code.value,
                    "verification_join_inviter": user_id,
                }
            },
        )
        await interaction.client.mongo.update_guild_member_document(
            user_id,
            {"$set": {f"verification_codes.{self.code.value}": interaction.user.id}},
        )

        bot_team_role = interaction.guild.get_role(
            interaction.client.config["role_id"]["bot_team_member"]
        )
        verified_member = interaction.guild.get_role(
            interaction.client.config["role_id"]["verified_member"]
        )
        ...
        await interaction.response.send_message(
            "You have successfully verified yourself as a bot team member.",
            ephemeral=True,
        )
...
class VerificationView(discord.ui.View):
    ...
    @discord.ui.button(
        ...
        style=discord.ButtonStyle.blurple,
        custom_id="persisten:lib_dev",
    )
    async def library_developer(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ) -> None:
        if await is_on_cooldown(interaction) is True:
            return
    ...
    @discord.ui.button(
        label="Bot Team Member",
        style=discord.ButtonStyle.blurple,
        custom_id="persisten:bot_team",
    )
    async def bot_team(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ) -> None:
...
class Verification(commands.Cog):
    ...
    async def after_ready(self) -> None:
        ...
        self.client.add_view(
            VerificationView(), message_id=guild_data["verification_message_id"]
        )
    ...
    async def send_verification_view(self, channel, **kwargs) -> None:
        ...
        msg = await channel.send(
            embed=verification_embed, view=VerificationView(), **kwargs
        )
    ...
    async def codes(self, interaction: discord.Interaction):
        ...
        if (
            interaction.user.get_role(
                self.client.config["role_id"]["verified_bot_developer"]
            )
            is None
        ):
            return await interaction.response.send_message(
        ...
        for role in interaction.user.roles:
            role_codes = self.client.config["role_id"]["bot_owner_roles"].get(
                str(role.id), 0
            )
        ...
        if len(guild_member["verification_codes"]) < total_codes:
            ...
            await self.client.mongo.update_guild_member_document(
                interaction.user.id,
                {"$set": {"verification_codes": guild_member["verification_codes"]}},
            )
        ...
        if len(description) == 0:
            description = (
                "You currently aren't qualified for any staff codes. Staff codes will be automatically "
                "unlocked when your bot reaches more servers."
            )
        else:
            description = (
                "Invite members from your bot team using the following code(s):\n\n"
                + description
            )

        codes_embed = discord.Embed(
            title="Codes", description=description, color=self.client.color
        )
    ...
    team_group = app_commands.Group(
        name="team", description="Manage your team on the server."
    )
    ...
    async def team_remove(self, interaction: discord.Interaction, user: discord.Member):
        ...
        if user.id not in guild_member["verification_codes"].values():
            return await interaction.response.send_message(
                "You did not invite this user.", ephemeral=True
            )

        await user.kick(
            reason=f"Removed by {interaction.user} ({interaction.user.id}), from their bot team."
        )
        await interaction.response.send_message(
            f"You have successfully removed {user.mention} from your team.",
            ephemeral=True,
        )
    ...
    async def team_view(
        ...
        if (
            user.get_role(self.client.config["role_id"]["verified_bot_developer"])
            is None
        ):
            return await interaction.response.send_message(
    ...
    async def on_member_remove(self, member: discord.Member):
        ...
        await self.client.mongo.update_guild_member_document(
            inviter_id, {"$unset": {f"verification_codes.{code}": ""}}
        )
